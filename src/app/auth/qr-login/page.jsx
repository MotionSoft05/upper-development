...
export default function QRLogin() {
  ...
  const validateSession = async (sessionId) => {
    try {
      ...
      console.log("🔍 Looking in collection: qr_auth_sessions");

      const sessionRef = doc(db, "qr_auth_sessions", sessionId);
      console.log("🔍 Document reference created:", sessionRef.path);

      const sessionDoc = await getDoc(sessionRef);
      console.log("🔍 Document exists?", sessionDoc.exists());

      if (!sessionDoc.exists()) {
        console.log("❌ Session document not found");
        console.log("❌ Document path:", sessionRef.path);
      ...
      setError("Error al validar la sesión: " + error.message);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="max-w-md w-full bg-white rounded-lg shadow-md p-6 text-center">
          ...
          <p className="text-sm text-gray-500">
            Por favor, genera un nuevo código QR desde tu TV e intenta
            nuevamente.
          </p>
          <Link
            href="/"
