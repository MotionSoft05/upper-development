...
} = require("./triggers/onPantallaConfigChange");
...
exports.updateFlights40Min = onSchedule(
  ...
  async (event) => {
    console.log(
      "🚀 Iniciando actualización unificada (vuelos + distancias)...",
    );
    ...
    try {
      ...
      console.log(
        `✅ Distancias actualizadas: ${distanceResults.processed} hoteles procesados`,
      );
    } catch (error) {
    ...
    try {
      ...
      if (!validAirports.includes(airport.toUpperCase())) {
        return res.status(400).json({
          success: false,
          error:
            `Aeropuerto ${airport} no soportado. ` +
            `Aeropuertos válidos: ${validAirports.join(", ")}`,
        });
      }

      const flightData = await flightService.getFlightData(
        airport.toUpperCase(),
      );
...
exports.getFlightData = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const data = doc.data();
      const lastUpdate =
        data.serverTimestamp?.toDate() || new Date(data.lastUpdate);
      ...
      console.error(
        `❌ Error obteniendo datos para ${airport}:`,
        error.message,
      );
...
exports.calculateTravelTime = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      const { hotelLocation, airportCode, options } =
...
exports.calculateMultipleRoutes = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      const { hotelLocation, airportCodes, options } =
        req.method === "POST" ? req.body : req.query;
      ...
      console.log(
        `🗺️ ${result.summary.successful}/${result.summary.total} rutas calculadas exitosamente`,
      );
...
exports.getAirportsInfo = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error(
        "❌ Error obteniendo información de aeropuertos:",
        error.message,
      );
      res.status(500).json({
        success: false,
  ...
  async (req, res) => {
    try {
        ...
        .set(
          {
            ...
            safeMode: true,
            note:
              "Sistema inicializado en modo pausado por seguridad. " +
      ...
      res.json({
        ...
        note:
          "Use el panel de administración para activar " +
...
exports.cronControl = onRequest(
  ...
  async (req, res) => {
    try {
      ...
      if (action === "restart") {
        console.log(
          "🔄 Restart solicitado - Ejecutando actualización inmediata...",
        );
        ...
        try {
          ...
          const airports = await getActiveAirports();
          console.log(
            `📡 Procesando aeropuertos activos: ${airports.join(", ")}`,
          );
          const flightResults = [];
          ...
          for (const airport of airports) {
            try {
              ...
              console.log(
                `✅ ${airport}: ${flightData.totalFlights} vuelos (${flightData.source})`,
              );
            } catch (error) {
              console.error(`❌ Error actualizando ${airport}:`, error.message);
              flightResults.push({
          ...
          console.log(
            `✅ Actualización manual completada en ${executionTime}ms`,
          );
          ...
          await db.collection("systemLogs").add({
            level: "info",
            message:
              `Actualización manual ejecutada: vuelos ${flightSummary.success ? "OK" : "ERROR"}, ` +
              `distancias ${distanceResults.success ? "OK" : "ERROR"}`,
            timestamp: admin.firestore.FieldValue.serverTimestamp(),
  ...
  async (req, res) => {
    try {
      ...
      await db.collection("systemLogs").add({
        level: "info",
        message:
          `Actualización manual (botón): vuelos ${flightSummary.success ? "OK" : "ERROR"}, ` +
          `distancias ${distanceResults.success ? "OK" : "ERROR"}`,
        timestamp: admin.firestore.FieldValue.serverTimestamp(),
      ...
      console.log(
        `📊 Vuelos: ${flightSummary.successful}/${airports.length} exitosos`,
      );
      ...
      res.json({
        success: true,
        message:
          "Actualización manual completada sin afectar el estado del cron",
        summary: {
          flightAirports: airports.length,
          flightSuccessful: flightResults.filter((r) => r.success).length,
          hotelsProcessed: distanceResults.processed || 0,
  ...
  async (req, res) => {
    try {
      ...
      const activeAirports =
        await hotelDistanceService.detectActiveAirports(companyId);
      ...
      if (!location) {
        ...
        location =
          await hotelDistanceService.getHotelLocationFromTemplates(companyId);
        ...
        if (!location) {
          ...
          console.log(
            `⚠️ Hotel ${companyId} no encontrado en TemplateVuelos, usando ubicación de fallback`,
          );
        }
      }
...
exports.getHotelDistanceData = onRequest(
  ...
  async (req, res) => {
    try {
      ...
      const distanceData =
        await hotelDistanceService.getHotelDistanceData(companyId);
...
exports.calculateHotelDistances = onRequest(
  ...
  async (req, res) => {
    try {
      ...
      const result =
        await hotelDistanceService.calculateHotelDistances(companyId);
...
exports.onFlightScreenConfigChange = onDocumentWritten(
  ...
  async (event) => {
    try {
      ...
      console.log(
        `🔄 Trigger activado para configuración de pantallas: ${companyId}`,
      );
      ...
      console.error(
        "❌ Error en trigger de configuración de pantallas:",
        error,
      );
      return null;
    }
...
exports.onCompanyConfigChange = onDocumentWritten(
  ...
  async (event) => {
    try {
      ...
      console.log(
        `🔄 Trigger activado para configuración de compañía: ${companyId}`,
      );
...
exports.toggleAirportService = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.log(
        `✅ ${airport} ${newState ? "activado" : "pausado"} exitosamente`,
      );
