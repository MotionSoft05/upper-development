 ...
 * 
 * Servicio para calcular tiempos de viaje en tiempo real entre hoteles y aeropuertos
 ...
 * @returns {Promise<Object>} Resultado con distancia, tiempo y detalles
 */
async function calculateTravelTime(hotelLocation, airportCode, options = {}) {
  try {
    ...
    const now = new Date();
    const departureTime = options.departureTime || Math.floor(now.getTime() / 1000);
    ...
    console.log(`🗺️ Calculando ruta: Hotel (${hotelLocation.lat}, ${hotelLocation.lng}) → ${airportCode}`);

    const response = await fetch(url);
    const data = await response.json();

    if (data.status !== 'OK') {
      throw new Error(`Google Distance Matrix API error: ${data.status} - ${data.error_message}`);
    }
    ...
    const result = {
      origin: {
        address: hotelLocation.address || `${hotelLocation.lat}, ${hotelLocation.lng}`,
        coordinates: {
...
async function calculateMultipleRoutes(hotelLocation, airportCodes = ["MEX", "TLC", "NLU"], options = {}) {
  try {
    ...
    const fastest = successful.reduce((prev, current) => {
      const prevTime = prev.data?.durationInTraffic?.value || prev.data?.duration?.value || Infinity;
      const currentTime = current.data?.durationInTraffic?.value || current.data?.duration?.value || Infinity;
...
function calculateRecommendedDeparture(routeData) {
  const baseTime = routeData.durationInTraffic?.minutes || routeData.duration.minutes;
  ...
  return {
    domestic_flight: {
      recommended_departure_before_flight: baseTime + factors.buffer + factors.checkin_domestic + factors.traffic_buffer,
      breakdown: {
    ...
    international_flight: {
      recommended_departure_before_flight: baseTime + factors.buffer + factors.checkin_international + factors.traffic_buffer,
      breakdown: {
        travel_time: baseTime,
        traffic_buffer: factors.traffic_buffer,
